...
module.exports = async (location, objects) => {
	const keys = objects.map(object => object.Key);

	let issues = await Promise.all(keys.map(issue => s3.getObjectFrom(location, issue)));
	...
	return `:exclamation: *Issue activity since ${weekday} morning*\n`
					+ listIssues('closed')
					+ listIssues('edited')
					+ listIssues('opened')
					+ listIssues('reopened');
